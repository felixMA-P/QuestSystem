...
namespace
{
	const TArray<FLinearColor>& GetDialogBranchPalette()
	{
...
FLinearColor UDialogGraphNodeBase::GetBranchColor(int32 Index)
{
	const TArray<FLinearColor>& Palette = GetDialogBranchPalette();
	return Palette[Index % Palette.Num()];
}

void UDialogGraphNodeBase::TagPinWithBranchColor(UEdGraphPin* Pin, const FLinearColor& Color)
{
	if (!Pin) return;
	Pin->PinType.PinSubCategory = FName(*(BranchPinPrefix + Color.ToFColor(true).ToHex()));
}

bool UDialogGraphNodeBase::TryGetBranchColor(const FEdGraphPinType& PinType, FLinearColor& OutColor)
{
...
void UDialogGraphNodeBase::SyncInputPinColors()
{
